...
def main():
    ...
    if CNN == False:
        ...
        print("Cnn")
        model = BILSTM_CNN(EMBEDDING_DIM, HIDDEN_DIM, len(word_to_ix), len(tag_to_ix), len(char_to_ix), emb_mat, CNN=True,use_gpu=use_gpu,BIDIRECTIONAL=BIDIRECTIONAL)
    if use_gpu:
        model = model.cuda()
    ...
    for epoch in range(500):
        ...
        for ind in range(int(len_train)):
            ...
            if ind%20 == 0:
                if SENNA:
                    get_results('text/train_ner_bilstm_cnn', model, training_data, y, ind, idx_to_tag, word_to_ix, tag_to_ix, char_to_ix, CNN, use_gpu)
                    get_results('text/test_ner_bilstm_cnn', model, test_X, test_y, ind, idx_to_tag, word_to_ix, tag_to_ix, CNN, use_gpu)
                else:
                    get_results('bioes_glove_text/train_ner_lstm', model, training_data, y, epoch, idx_to_tag, word_to_ix, tag_to_ix,char_to_ix, CNN, use_gpu)
                    get_results('bioes_glove_text/test_ner_lstm', model, test_X, test_y, epoch, idx_to_tag, word_to_ix, tag_to_ix, char_to_ix, CNN, use_gpu)
