...
def load_duolingo_seq_feats(label):
    if label == 'train':
        with open('../data/duolingo/'+lang_pair+'/'+lang_pair+'_train_seq_feats.pkl', 'rb') as f:
            seq_feats = pickle.load(f)
        print("Train data length: " + str(len(seq_feats['countries'])))

    elif label == 'test' or label == 'dev':
        with open('../data/duolingo/'+lang_pair+'/'+lang_pair+'_dev_seq_feats.pkl', 'rb') as f:
            seq_feats = pickle.load(f)
...
def process_duolingo_seq_feats(train_feats, test_feats):
    ...
    for key in train_feats.keys():
        ...
        if key in exceptions:  # exceptions does not have i2w mapping
            if key == 'days':
                ...
                print('{} vocab size: {} where {} is the OOV index'.format(key, len(unique_vals) + 2,
                                                                           len(unique_vals) + 1))

            elif key == 'time':  # time: the amount of time (in seconds) it took for the student to construct and
        ...
        with open('../data/duolingo/'+lang_pair+'/i2t_' + ("country" if key == "countries" else key) + '.pkl', 'wb') as f:
            i2t = dict()

            for i, val in enumerate(train_feats[key]):
                train_feats[key][i] = unique_vals.index(val) + 1
                i2t[train_feats[key][i]] = val
...
if __name__ == '__main__':
    ...
    new_train_feats = [
        {'user': train_feats['user'][i], 'country': train_feats['countries'][i], 'days': train_feats['days'][i],
         'client': train_feats['client'][i], 'session': train_feats['session'][i], 'format': train_feats['format'][i],
         'time': train_feats['time'][i]} for i in range(len(train_feats['countries']))]

    new_dev_feats = [{'user': dev_feats['user'][i], 'country': dev_feats['countries'][i], 'days': dev_feats['days'][i],
                      'client': dev_feats['client'][i], 'session': dev_feats['session'][i],
                      'format': dev_feats['format'][i], 'time': dev_feats['time'][i]} for i in
                     range(len(dev_feats['countries']))]

    with open('../data/duolingo/'+lang_pair+'/procd_'+lang_pair+'_train_seq_feats.pkl', 'wb') as f:
        pickle.dump(new_train_feats, f)

    with open('../data/duolingo/'+lang_pair+'/procd_'+lang_pair+'_dev_seq_feats.pkl', 'wb') as f:
